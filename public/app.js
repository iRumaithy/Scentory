...
const DEMO_VOTES={imagination:8200,dhi:14500,'oud-wood':19800,naxos:11700,aventus:31000,br540:24000,bleu:22200,layton:18100,reflection:9400,hacivat:12500,'bal-afrique':10200,'jazz-club':16400};
PERFUMES.forEach(p=>{p.votes=Number.isFinite(p.votes)?p.votes:(DEMO_VOTES[p.id]||0)});
...
function formatVotes(n){n=Number.isFinite(n)?n:0;if(n>=1000)return (n/1000).toFixed(n>=10000?0:1)+'k';return String(n)}
  ...
  $('#detailContent').innerHTML=`<button class="detail-mobile-close" id="closeDetail">×</button><article class="detail-card">
    <div class="detail-top">
      ...
      <div><div class="detail-brand">${p.brand.toUpperCase()}</div><h2 class="detail-name">${p.name.toUpperCase()}</h2><div class="detail-type">Eau de Parfum</div>
        ...
        <div class="detail-rating"><strong>${p.rating.toFixed(1)}</strong><span>${stars()}</span><small>(${(Number.isFinite(p.votes)?p.votes:0).toLocaleString('en-US')})</small></div>
      </div>
    </div>
